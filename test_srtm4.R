...
#Parallel setup
foldK <- 10
nCor <- 10 #5 cores por motivos de memoria (a metade de foldK)
...
samp_random <- function(r, p) {
  er <- extent(r)
  N <- cellStats(r, function (x,na.rm=F) sum(!is.na(x))) #r@ncols * r@nrows
  Np <- round(p*N)
...
for( t_ty in c("random") ) { # "random", "transpar", "randlow", "transects"
  
  #Test different point densities
  for( n_p in c(6,5,7,4,8) ) {
...
if(FALSE) {        
    ...
    cat("Preparing", paste(sId0, "-fr", ".tif", sep=""), "\n")
    
    #load stack of simple interpolations
    if( ! file.exists(paste("data", paste(sId0, "-fr", ".tif", sep=""), sep="/")) ) {
      fr.ls <- paste("data", paste(paste(sId0, seq(1,foldK), sep="-"), "-fr", ".asc", sep=""), sep="/")
      if( file.exists( fr.ls[1] ) ) {
        frs <- stack(fr.ls)
        proj4string(frs) <- readLines( paste("data", paste(sId0, "-1-fr", ".crs", sep=""), sep="/") )
      } else {
      ...
      ss <- projectRaster( raster(paste("data", "srtm2.tif", sep="/")), frs)
        frs[ is.na(ss) ] <- NA
      writeRaster(frs, paste("data", paste(sId0, "-fr", ".tif", sep=""), sep="/"), overwrite=T )
    } else {
      frs <- raster(paste("data", paste(sId0, "-fr", ".tif", sep=""), sep="/"))
      ss <- projectRaster( raster(paste("data", "srtm2.tif", sep="/")), frs)
    }
    ...
    cat("Preparing", paste(sId0, "-ffr", ".tif", sep=""), "\n")
    
    #load stack of fractal interpolations
    if( ! file.exists(paste("data", paste(sId0, "-ffr", ".tif", sep=""), sep="/")) ) {
      ffr.ls <- paste("data", paste(paste(sId0, seq(1,foldK), sep="-"), "-ffr", ".asc", sep=""), sep="/")
      if( file.exists( ffr.ls[1] ) ) {
        ffrs <- stack(ffr.ls)
        proj4string(ffrs) <- readLines( paste("data", paste(sId0, "-1-ffr", ".crs", sep=""), sep="/") )
      } else {
      ...
      ss <- projectRaster( raster(paste("data", "srtm2.tif", sep="/")), ffrs)
        ffrs[ is.na(ss) ] <- NA
      writeRaster(ffrs, paste("data", paste(sId0, "-ffr", ".tif", sep=""), sep="/"), overwrite=T )
    } else {
      ffrs <- raster(paste("data", paste(sId0, "-ffr", ".tif", sep=""), sep="/"))
      ss <- projectRaster( raster(paste("data", "srtm2.tif", sep="/")), ffrs)
    }    
